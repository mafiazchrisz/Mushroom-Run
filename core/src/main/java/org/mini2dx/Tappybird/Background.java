...
public class Background extends Hazards {
    ...
    private static float BACKGROUND_SPEED = 0.3f; // Default = 0.3f
    private float width;
    ...
    void update() {
        ...
        if (point.getX() < -width + 201) {
            point.setX(width - 201);
        }
    }

    void render(Graphics g) {
        g.drawTexture(backgroundTexture.background, point.getX(), point.getY());
    }
}
